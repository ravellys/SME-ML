...
The ETa is the major water flux in hydrologycal cicle.
The of ETa can be obtained by the Eddy Covariance method (EC), however, this method is so expensive.

This code show a new approach to estimate the ETa by Machine Learning.

We train the model with soil moisture, rainfall and potential Evapotranspiration data (calculated by Pean-Monteith).
"""
#initial soil moistures
...
#separete train and test data to input in model
x_train, x_test, y_train_eta, y_test_eta = train_test_split(x_, y_eta, 
                                                    test_size=0.5,
                                                    shuffle = False)
...
gp_eta = GaussianProcessRegressor(alpha=1e-6,
                                  kernel=kernel,
                                  n_restarts_optimizer=10, 
                                  normalize_y=True,
                                  optimizer='fmin_l_bfgs_b')
...
ETa = eta_o.tolist()[0]
for i in range(len(y_)):
    #ETa.append(gp_eta.predict(np.array([x_[i,:-1]]))[0,0])
